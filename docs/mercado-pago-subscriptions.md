...
| `PUBLIC_SITE_URL` | Yes (prod) | Base URL used for the `back_url` redirect back to `/pt-br/checkout/complete/`. Falls back to the `SITE_ORIGIN` constant with a loud server-side warning if unset. |
...
> Note: `.env`/`.env.local` currently contain the production
> `MERCADO_PAGO_ACCESS_TOKEN` plus stale `TURSO_*` variables left over from a
> removed database stack — the Turso variables are unused and can be deleted.
...
- `idempotencyKey` — a UUID generated once per checkout session and reused
  across retries; sent as Mercado Pago's `X-Idempotency-Key` so a double-click
  or a retry after a network drop cannot create duplicate subscriptions (no
  database involved).
...
`/pt-br/checkout/complete/` — a simple prerendered thank-you page
(`Obrigado. Sua assinatura foi processada pelo Mercado Pago...`). It does not
persist any local state and does not trust browser query parameters as proof of
payment. If an authoritative subscription status is ever needed on the site,
query Mercado Pago directly (see below) — do not build local payment-state
persistence.
...
A sanitized status proxy exists server-side for future use:
`getSubscription(id)` in `src/lib/server/mercadoPago.ts` returns only
`id`, `status`, `reason`, `external_reference`, `payer_email`,
`transaction_amount`, `currency_id` — nothing is cached locally.
...
- Duplicate submissions: client button disabled while pending + MP
  `X-Idempotency-Key` from a per-attempt UUID.
- No local database → no rate limiter on checkout creation (documented
  trade-off; add one behind an external store if abuse appears).
