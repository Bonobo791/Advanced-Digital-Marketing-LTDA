<script lang="ts">
  ...
  import { EMAIL, MAILTO, PAGE_COPY, PORTUGUESE_EMAIL, PT_MAILTO, WHATSAPP_AVAILABLE, WHATSAPP_URL } from '$lib/constants'
...
<div class="index-home" class:portuguese={locale === 'pt-BR'}>
  ...
  <section class="contact" id="contact"><div class="kanji" style="right:-5vw;bottom:-16%" aria-hidden="true">連絡</div><div class="sec-inner"><span class="sec-jp rise" style="color:var(--ink)">{content.contactLabel}<span class="font-jp">連絡</span></span><h2 class="shear">{#each contactWords as word, i}<span class="w">{word}{i < contactWords.length - 1 ? ' ' : ''}</span>{/each}</h2><p class="c-sub rise">{content.contactSub}</p>{#if showWhatsapp}<a class="cta-hanko rise" href={WHATSAPP_URL}><span class="seal font-jp">答</span><span class="txt">{content.whatsapp}</span></a><a class="c-mail rise" href={localeMailto}><span>{content.emailCta}</span></a>{:else}<a class="cta-hanko rise" href={localeMailto}><span class="seal font-jp">答</span><span class="txt">{content.email}</span></a><a class="c-mail rise" href={localeMailto}><span>{localeEmail}</span></a>{/if}</div></section>
</div>
