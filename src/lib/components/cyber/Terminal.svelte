...
<div class="bracketed chamfer border border-white/10 bg-[#1d252c] {className}">
  ...
  <div class="px-4 py-4 font-mono2 text-[13px] leading-[1.9] text-white/80">
    {#each typed as l, i (i)}
      <div>
        {#if l?.prompt}<span class="text-[#76d7dd]">{l.prompt} </span>{/if}{l?.text}
      </div>
    {/each}
    {#if !done && active}
      <div>
        {#if active.prompt}<span class="text-[#76d7dd]">{active.prompt} </span>{/if}{current}<span
          class="cursor-block"
        ></span>
      </div>
    {/if}
    {#if done}
      <div>
        <span class="text-[#76d7dd]">&gt; </span><span class="cursor-block"></span>
      </div>
    {/if}
  </div>
</div>
